...
(() => {
  ...
  function syncLedger(state) {
    ...
    if (hasActiveTrade) {
      const locked = Math.max(
        0,
        money(activeTrade.investment)
      );

      state.lockedBalance =
        Number(locked.toFixed(2));
  ...
  function start(planId, amount) {
    ...
    /*
     * Populate the activity feed immediately.
     */
    ...
    /*
     * Principal remains inside total balance,
     * but becomes locked.
     */
    state.lockedBalance =
      Number(
  ...
  function completeTrade(
    ...
    /*
     * The principal was locked, not removed
     * from total balance.
     *
     * Add only the profit.
     */
  ...
  function tick() {
    ...
    /*
     * Accumulate profit during the cycle.
     */
    trade.currentProfit =
      Number(
        (
  ...
  function render() {
    ...
    /*
     * Show total balance plus
     * currently accumulating profit
     * while the trade is running.
     */
    const liveEquity =
      trade?.status === 'active'
        ? Number(
